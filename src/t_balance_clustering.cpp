...
void TBC::updateClusterCenters() {
    ...
    for (auto &clusterCenter: clusterCenters) {
        ...
        while (!clusterCenter.cluster.empty()) {
            std::pair<int, double> idxDistPair = clusterCenter.cluster.top();
            std::vector<double> dataValue = data_[idxDistPair.first];
...
//    std::priority_queue<std::pair<double, ClusterCenter>, std::vector<std::pair<double, ClusterCenter>>, MinHeap> nearestClusters;
    ...
    std::priority_queue<std::pair<double, int>, std::vector<std::pair<double, int>>, MinHeap> nearestClusters;

    for (int i = 0; i < clusterCenters.size(); ++ i) {
        // 计算数据点与簇中心的距离
        double dist = distance(clusterCenters[i].value, dataItem);
        // 将数据点与簇中心构成的 <簇中心, 与簇中心距离> 对加入最小堆
...
//        std::cout << "cluster size: " << nearest.cluster.size() << std::endl;
        ...
        if (nearest.cluster.size() <= scope_) {
            nearest.cluster.push(std::make_pair(dataIndex, dist));
...
//            return std::vector<double>();
            ...
            // 取得边界点的索引和与簇中心的距离
            std::pair<int, double> boundary = nearest.cluster.top();

            // 如果边界点比当前点远，
            if (boundary.second > dist) {
                // 则将边界点踢出当前簇
                nearest.cluster.pop();
                // 并加入新的点
                nearest.cluster.push(std::make_pair(dataIndex, dist));
...
//            return data_[boundary.first];
                assignToNearestCenter(boundary.first, data_[boundary.first]);
                return;
            }
        }
        nearestClusters.pop();
